...
public abstract class AbstractJobConfigurationGsonTypeAdapter<T extends JobRootConfiguration> extends TypeAdapter<T> {
    ...
    public T read(final JsonReader in) throws IOException {
        ...
        boolean jobExecutionLog = false;
        boolean jobStatusTraceLog = false;
        ...
        while (in.hasNext()) {
            ...
            switch (jsonName) {
                ...
                case "jobExecutionLog":
                	jobExecutionLog = in.nextBoolean();
                    break;
                //新增扩展字段
                case "jobStatusTraceLog":
                	jobStatusTraceLog = in.nextBoolean();
                    break;
                //修复版本原来的BUG
        ...
        JobCoreConfiguration coreConfig = getJobCoreConfiguration(jobName, cron, shardingTotalCount, shardingItemParameters,
                jobParameter, failover, misfire, description, jobProperties,jobExecutionLog,jobStatusTraceLog);
    ...
    private JobCoreConfiguration getJobCoreConfiguration(final String jobName, final String cron, final int shardingTotalCount,
                                                         ...
                                                         final JobProperties jobProperties,final boolean jobExecutionLog,final boolean jobStatusTraceLog) {
        return JobCoreConfiguration.newBuilder(jobName, cron, shardingTotalCount)
                .shardingItemParameters(shardingItemParameters).jobParameter(jobParameter).failover(failover).misfire(misfire).jobExecutionLog(jobExecutionLog).jobStatusTraceLog(jobStatusTraceLog).description(description)
    ...
    public void write(final JsonWriter out, final T value) throws IOException {
        ...
        out.name("jobExecutionLog").value(value.getTypeConfig().getCoreConfig().isJobExecutionLog());
        out.name("jobStatusTraceLog").value(value.getTypeConfig().getCoreConfig().isJobStatusTraceLog());
